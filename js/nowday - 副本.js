...
document.getElementById("picshow").src = 'https://api.berryapi.net/?service=App.Bing.Images&day=-0'; // 初始化今日图片
...
function nextday() {
    var newdayc = day_up()
    var update = document.getElementById("date");
    if (newdayc > trueday){ // 请求图片日期未存在
        day_down()
        alert('未发布')
    }/* 
    else if(newdayc == showDate){
        // alert(showDate)
        var newday = newdayc.Format("yyyy-MM-dd")
        document.getElementById("picshow").src = 'https://api.berryapi.net/?service=App.Bing.Images&day=-0';
        update.innerHTML = newday
    }
     */else{
        var newday = newdayc.Format("yyyy-MM-dd")
